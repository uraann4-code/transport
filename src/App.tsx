import React, { useRef } from 'react';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';
import { Download } from 'lucide-react';

export default function App() {
  const formRef = useRef<HTMLDivElement>(null);

  const handleDownloadPdf = async () => {
    if (!formRef.current) return;
    
    try {
      const canvas = await html2canvas(formRef.current, {
        scale: 2,
        useCORS: true,
        logging: false
      });
      
      const imgData = canvas.toDataURL('image/png');
      const pdf = new jsPDF({
        orientation: 'portrait',
        unit: 'px',
        format: [canvas.width, canvas.height]
      });
      
      pdf.addImage(imgData, 'PNG', 0, 0, canvas.width, canvas.height);
      pdf.save('BU_Transport_Requisition_Form.pdf');
    } catch (error) {
      console.error("Error generating PDF", error);
    }
  };

  return (
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col items-center justify-center p-4 sm:p-10 font-sans text-[#111827] overflow-y-auto">
      <div className="w-full max-w-[920px] flex justify-end mb-4">
        <button 
          onClick={handleDownloadPdf}
          className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white text-xs font-bold rounded shadow-lg shadow-blue-200 uppercase tracking-widest hover:bg-blue-700 transition-colors"
        >
          <Download size={16} />
          Download & Print
        </button>
      </div>
      <div ref={formRef} className="bg-white shadow-2xl rounded-xl border border-gray-200 max-w-[920px] w-full p-8 sm:p-10 flex flex-col gap-6 relative">
        
        {/* Header */}
        <div className="flex flex-col text-center border-b border-gray-200 pb-6 mb-2">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-2">BU TRANSPORT REQUISITION FORM (WITHIN STATION)</h1>
          <p className="text-sm text-gray-500 uppercase tracking-widest font-medium italic">(To reach the Transport Officer in duplicate not less than 24 hours in advance).</p>
        </div>

        {/* Section 1 */}
        <div className="flex flex-col gap-6">
          <div className="flex items-center gap-4">
            <span className="text-[11px] font-bold text-gray-600 w-4">a.</span>
            <span className="text-[11px] font-bold uppercase text-gray-600">Transport Officer</span>
          </div>
          
          <div className="flex items-center gap-4">
            <span className="text-[11px] font-bold text-gray-600 w-4">b.</span>
            <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap">MT pool of</span>
            <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 w-64" />
          </div>
          
          <h2 className="text-xs font-black uppercase tracking-[0.2em] text-blue-600 flex items-center gap-2 mt-2">
            <span className="w-4 h-[1px] bg-blue-600"></span> Request transport be provided as under:
          </h2>

          <div className="grid grid-cols-1 gap-y-4 mt-2">
            <div className="flex items-center gap-4">
              <span className="text-[11px] font-bold text-gray-600 w-4">c.</span>
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-64">Date and time on which required</span>
              <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
            </div>
            
            <div className="flex items-center gap-4">
              <span className="text-[11px] font-bold text-gray-600 w-4">d.</span>
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-64">Type of vehicle required</span>
              <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
            </div>

            <div className="flex items-center gap-4">
              <span className="text-[11px] font-bold text-gray-600 w-4">e.</span>
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-64">Nature of Duty</span>
              <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
            </div>

            <div className="flex items-center gap-4">
              <span className="text-[11px] font-bold text-gray-600 w-4">f.</span>
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-64">Destination with routes</span>
              <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
            </div>

            <div className="flex items-center gap-4">
              <span className="text-[11px] font-bold text-gray-600 w-4">g.</span>
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-64">Duration for which required</span>
              <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
            </div>
          </div>
        </div>

        {/* Signatures part 1 */}
        <div className="flex flex-col items-end mt-8 mb-4 space-y-4">
           <div className="flex items-center gap-4 w-80">
             <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-24">Signature</span>
             <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
           </div>
           <div className="flex items-center gap-4 w-80">
             <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap w-24">Designation</span>
             <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
           </div>
        </div>

        {/* Part II */}
        <div className="mt-8 pt-10 border-t border-gray-200 flex flex-col gap-6">
          <p className="text-[11px] font-bold uppercase text-gray-400 text-center tracking-widest">For use of Transport Officer</p>
          <div className="text-center text-xs font-black uppercase tracking-[0.2em] text-blue-600 flex justify-center items-center gap-2 mb-2">
            <span className="w-8 h-[1px] bg-blue-600"></span> PART-II <span className="w-8 h-[1px] bg-blue-600"></span>
          </div>
          
          <div className="space-y-6">
            <div className="flex items-center gap-4 w-full">
              <span className="text-[11px] font-bold text-gray-600 w-4">a.</span>
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap">Transport No.</span>
              <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
              <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap">has been booked.</span>
            </div>

            <div className="flex items-center gap-4 w-full">
               <span className="text-[11px] font-bold text-gray-600 w-4">b.</span>
               <span className="text-[11px] font-bold uppercase text-gray-600 whitespace-nowrap">Transport not available due to</span>
               <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 flex-grow" />
            </div>
            
            <div className="w-full pl-8">
               <input type="text" className="h-10 border border-gray-300 rounded bg-gray-50 px-3 text-sm focus:outline-none focus:border-blue-600 w-full" />
            </div>
          </div>
        </div>

        {/* MTO Signature */}
        <div className="flex justify-end mt-12 mb-8 mr-4">
          <div className="flex flex-col items-center gap-2">
            <div className="w-48 h-12 border-b border-gray-900 relative">
              <input type="text" className="absolute bottom-1 w-full text-center bg-transparent focus:outline-none font-serif italic text-lg text-gray-800" placeholder="" />
            </div>
            <label className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">MTO</label>
          </div>
        </div>

        {/* Approval */}
        <div className="text-center mt-12 mb-12 flex justify-center">
           <span className="text-[11px] font-bold uppercase tracking-[0.1em] text-gray-700 bg-gray-100 px-8 py-3 rounded-md">
             Approved &nbsp;/&nbsp; Not Approved
           </span>
        </div>

        {/* Director Signature */}
        <div className="flex justify-center mt-8 pt-8">
          <div className="flex flex-col items-center gap-2">
            <div className="w-64 h-12 border-b border-gray-900 relative">
              <input type="text" className="absolute bottom-1 w-full text-center bg-transparent focus:outline-none font-serif italic text-lg text-gray-800" placeholder="" />
            </div>
            <label className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Director</label>
          </div>
        </div>

        {/* Page number */}
        <div className="text-center mt-16 text-[10px] text-gray-400 font-bold">
          11
        </div>
      </div>
    </div>
  );
}
